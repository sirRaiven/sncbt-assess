...
function logBackendError(
  ...
  console.error(
    `[classrooms:${operation}] backend operation failed.`,
    error,
  );
}
...
async function updateClassroom(
  ...
  if (error) {
    ...
    throw new AppError(
      ...
      "The class information could not be updated.",
    );
  }
...
async function regenerateJoinCode(
  ...
  if (error) {
    ...
    throw new AppError(
      ...
      "A new class code could not be generated.",
    );
  }
...
async function changeCodeState(
  ...
  if (error) {
    ...
    throw new AppError(
      ...
      "The class code setting could not be updated.",
    );
  }
...
async function getEnrollmentSettings(
  ...
  if (pendingError) {
    ...
    throw new AppError(
      500,
      "ENROLLMENT_SETTINGS_LOAD_FAILED",
      "The enrollment settings could not be loaded.",
    );
  }

  return response({
    joinEnabled:
      Boolean(classroom.join_enabled),
    requiresApproval:
      Boolean(
...
async function setEnrollmentApproval(
  admin: any,
  instructorId: string,
  classroomId: string,
  required: boolean,
  ...
  if (classroom.status !== "active") {
    throw new AppError(
      409,
      "CLASSROOM_ARCHIVED",
      "Reactivate the class before changing its enrollment approval setting.",
    );
  }
  ...
  const { error: updateError } = await admin
    .from("classrooms")
    .update({
      join_requires_approval: required,
    })
    .eq("id", classroomId)
  ...
  if (updateError) {
    ...
    throw new AppError(
      400,
      "ENROLLMENT_APPROVAL_UPDATE_FAILED",
      "The enrollment approval setting could not be updated.",
    );
  }
  ...
  if (pendingError) {
    logBackendError(
      "set-enrollment-approval:pending-count",
      pendingError,
    );
  }
  ...
  return response({
    ...
    message:
      required
        ? "Instructor approval is now required for new class-code requests."
        : "New valid class-code requests will now join automatically.",
  });
}
  ...
  if (existing) {
    ...
    if (error) {
      ...
      throw new AppError(
        ...
        "The class membership request could not be submitted.",
      );
    }
...
async function leaveMembership(
  ...
  if (error) {
    ...
    throw new AppError(
      400,
      "LEAVE_CLASS_FAILED",
      "The class could not be left right now.",
    );
  }
    ...
    async (
      ...
      try {
        ...
        switch (input.action) {
          ...
          case "set-enrollment-approval":
            requireRole(
              profile,
              "instructor",
            );

            return await setEnrollmentApproval(
              context.supabaseAdmin,
              userId,
              input.payload
                .classroomId,
